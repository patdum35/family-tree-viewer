...
// Fonction pour générer le fichier de géolocalisation



// export async function generateGeocodeFile() {
...
//     // Si un fichier de géolocalisation existe déjà, le charger d'abord
//     try {
...
//         try {
//             // Vérifier si on a déjà les coordonnées
...
//         processed++;
//         // Attendre seulement si on fait une nouvelle requête
//         if (!geolocalisationData[location]) {
//             await new Promise(resolve => setTimeout(resolve, 1000));
//         }
//     }
...
// Fonction pour normaliser les adresses
...
//         .replace(/\([^)]*\)/g, '')
//         // Supprimer les numéros département comme (01) ou [01]
//         .replace(/[\(\[]\d{2,3}[\)\]]/g, '')
//         // Supprimer les codes postaux au début
//         .replace(/^\d{4,5}\s+/, '')
//         // Remplacer multiples espaces par un seul
//         .replace(/\s+/g, ' ')
//         // Supprimer les virgules et points à la fin
//         .replace(/[,\.]$/, '')
//         .trim();

//     // Liste des mots inutiles à supprimer
//     const wordsToRemove = ['commune', 'ville', 'village', 'hameau', 'lieu-dit'];
//     wordsToRemove.forEach(word => {
//         normalized = normalized.replace(new RegExp(`\\b${word}\\b`, 'g'), '');
//     });

//     // Ajouter ", france" si on a juste une ville française
//     if (!normalized.includes('france') && !normalized.includes(',')) {
//         normalized += ', france';
//     }
...
//     // Collecter et normaliser tous les lieux uniques
//     const locationsMap = new Map(); // Map pour garder trace des lieux originaux
    
//     for (const person of Object.values(state.gedcomData.individuals)) {
//         [person.birthPlace, person.deathPlace].forEach(place => {
//             if (place) {
//                 const normalizedPlace = normalizeLocation(place);
//                 if (normalizedPlace) {
//                     if (!locationsMap.has(normalizedPlace)) {
//                         locationsMap.set(normalizedPlace, new Set());
//                     }
//                     locationsMap.get(normalizedPlace).add(place);
//                 }
//             }
//         });
...
//                     const normalizedPlace = normalizeLocation(family.marriagePlace);
//                     if (normalizedPlace) {
//                         if (!locationsMap.has(normalizedPlace)) {
//                             locationsMap.set(normalizedPlace, new Set());
//                         }
//                         locationsMap.get(normalizedPlace).add(family.marriagePlace);
//                     }
//                 }
//             });
//         }
...
//     // Convertir en array et limiter si nécessaire
//     let locationsArray = Array.from(locationsMap.keys());
//     if (maxLocations > 0) {
//         locationsArray = locationsArray.slice(0, maxLocations);
//     }
...
//     for (const normalizedLocation of locationsArray) {
//         statusDiv.textContent = `Géocodage: ${processed + 1}/${totalLocations} - ${normalizedLocation}`;
//         progressBar.value = (processed / totalLocations) * 100;

//         try {
//             let coords;
//             if (geolocalisationData[normalizedLocation]) {
//                 coords = geolocalisationData[normalizedLocation];
//                 resultsList.innerHTML += `<option style="color: blue;">↺ ${normalizedLocation} (cache)</option>`;
//             } else {
//                 coords = await geocodeLocation(normalizedLocation);
//                 if (coords) {
//                     // Stocker les coordonnées pour toutes les variantes
//                     const variants = locationsMap.get(normalizedLocation);
//                     variants.forEach(variant => {
//                         geolocalisationData[variant] = coords;
//                     });
//                     resultsList.innerHTML += `<option style="color: green;">✓ ${normalizedLocation} (${variants.size} variantes)</option>`;
//                 } else {
//                     resultsList.innerHTML += `<option style="color: red;">✗ ${normalizedLocation}</option>`;
//                 }
//             }
//         } catch (error) {
//             resultsList.innerHTML += `<option style="color: red;">✗ ${normalizedLocation} (${error.message})</option>`;
//         }
