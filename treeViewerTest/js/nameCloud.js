...
const getCenterCloudNameContainer = async () => {
    const { centerCloudNameContainer } = await import('./nameCloudRenderer.js'); // OK
    return centerCloudNameContainer;
};
...
import { getLoadSettingsFromLocalStorage } from './main.js';
...
console.log('\n\n\n  *** debug processNamesCloudWithDate start ', config, ', isCallFromCloudName=',isCallFromCloudName, ', nameData=', nameData, ', isNameDataIn=', isNameDataIn, '################ \n\n\n')
...
export async function processNamesCloudWithDateInternal(config, containerElement = null, isCallFromCloudName = false, nameDataIn = null, isNameDataIn = false) {
 

    const centerCloudNameContainer = await getCenterCloudNameContainer();
    ...
    const loadSettingsFromLocalStorage = await getLoadSettingsFromLocalStorage();
    ...
    const createNameCloudUI = await getCreateNameCloudUI();
    // Afficher le nuage
    if (containerElement) {
    ...
    window.addEventListener('resize', debounce(() => {
        ...
        setTimeout(async () => {
            if (state.isWordCloudEnabled) {
                ...
                const updateOverlayLayout = await getUpdateOverlayLayout();
                updateOverlayLayout();
                // resizeHamburger();
            }
        }, 150);
