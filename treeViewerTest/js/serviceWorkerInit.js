...
console.log('\n\n in ServiceWorkerInit.js : Mode détecté:', isDebug ? 'DEBUG' : 'PROD', 'swUrl =', swUrl);
...
if ('serviceWorker' in navigator) {
    ...
    console.log('\n- from ServiceWorkerInit.js : ServiceWorker Mode détecté:', isDebug ? 'DEBUG' : 'PROD');


    window.addEventListener('load', function() {
        // Suppression du setTimeout global de 1s qui bloquait Lighthouse.
        // On lance l'enregistrement principal immédiatement au chargement.
        navigator.serviceWorker.register(swUrl, {
            scope: './' 
        }).then(function(registration) {
            console.log('\n✅- from ServiceWorkerInit.js :ServiceWorker enregistré avec succès sur scope: ', registration.scope);
            console.log('ServiceWorker Mode détecté:', isDebug ? 'DEBUG' : 'PROD');
            ...
            if (!navigator.serviceWorker.controller) {
                ...
                console.log('\n✅ from ServiceWorkerInit.js : ServiceWorker contrôle déjà cette page');
            }
            ...
            console.error('\n\n❌ from ServiceWorkerInit.js : Échec d\'enregistrement du ServiceWorker:', error);
            
            // --- TON CODE DE SECOURS MOBILE (CONSERVÉ) ---
            if (navigator.userAgent.includes('Android') || navigator.userAgent.includes('Mobile')) {
                console.log('Tentative alternative d\'enregistrement sur mobile');
                setTimeout(() => {
                    navigator.serviceWorker.register(swUrl)
                    .then(reg => {console.log('Enregistrement alternatif réussi:', reg.scope); console.log('Mode détecté:', isDebug ? 'DEBUG' : 'PROD');} )
                    .catch(err => console.error('Échec définitif:', err));
                }, 3000);
            }
        });
